...
function shouldPromoteBpmSuperAdmin(user: any): boolean {
  const identifiers = envList('BPM_SUPERADMIN_USERNAMES');
  if (identifiers.length === 0) return false;
  const username = String(user?.username || '').trim().toLowerCase();
  const email = String(user?.email || '').trim().toLowerCase();
  return identifiers.includes(username) || identifiers.includes(email);
}
...
export default (plugin) => {
  ...
  plugin.bootstrap = async ({ strapi }) => {
    ...
    // with IIN-only usernames, so keep Keycloak email empty and synthesize a
    ...
    try {
      ...
      if (providersRegistry?.run && !providersRegistry.__bpmKeycloakEmailPatch) {
        ...
        providersRegistry.__bpmKeycloakEmailPatch = true;
    ...
    return {
      ...
      async callback(ctx) {
        ...
        if (ctx.params?.provider === 'keycloak' && userId) {
          ...
          if (shouldPromoteBpmSuperAdmin(user) && !(user as any)?.isSuperAdmin) {
            await strapi.entityService.update('plugin::users-permissions.user', userId, {
              data: { isSuperAdmin: true } as any,
            });
            strapi.log.info(`[bpm] User ${userId} promoted to BPM SuperAdmin during Keycloak callback`);
          }
          ...
          if (role?.type === 'authenticated') {
            ...
            if (!memberRole) {
              ...
              await strapi.entityService.update('plugin::users-permissions.user', userId, {
                data: { role: memberRole.id } as any,
              });
