  ...
  return (
    <div>
      ...
      >
        ...
        <Icon className={`h-4 w-4 shrink-0 ${isSelected ? 'text-teal-700' : 'text-slate-400'}`} />
      ...
      <section className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
        <aside className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Мои задачи</h2>
              <p className="text-xs text-slate-500">Реестр процессов</p>
            </div>
            <div className="flex items-center gap-1 rounded-md bg-slate-50 p-1">
              <button
                className="rounded bg-white px-2 py-1 text-xs font-semibold text-slate-500 shadow-sm"
                type="button"
        ...
        <main className="space-y-4">
          ...
          <section className="overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm">
            <div className="grid min-w-[720px] grid-cols-[1.4fr_160px_140px_120px] border-b border-slate-100 px-4 py-3 text-xs font-semibold uppercase text-slate-400">
              <span>Название</span>
              <span>Тип</span>
              <span>Состояние</span>
              <span>Год</span>
              ...
              {(selectedChildren.length ? selectedChildren : companyArchive(`${selectedNode.id}-empty`)).map((child) => {
                ...
                return (
                  <button
                    type="button"
                    key={child.id}
                    className="grid min-w-[720px] w-full grid-cols-[1.4fr_160px_140px_120px] items-center px-4 py-3 text-left text-sm transition hover:bg-slate-50"
                    onClick={() => selectNode(child)}
                  >
                    <span className="flex min-w-0 items-center gap-3 font-medium text-slate-800">
                      <ChildIcon className="h-4 w-4 shrink-0 text-slate-400" />
