...
export default function HelpdeskPage() {
  ...
  const sharedItViewer = ['ernar', 'zhandos'].some(
    (username) =>
      currentUser?.username?.toLowerCase() === username ||
      currentUser?.email?.toLowerCase() === `${username}@nnmc.kz`
  );
  const canFilterByAssignee = isSuperAdmin || isKuat || sharedItViewer;
  ...
  const [myTicketsOnly, setMyTicketsOnly] = useState(!sharedItViewer);
